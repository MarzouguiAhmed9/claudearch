---
name: rag-latency-spec
description: Spec for reducing end-to-end RAG response latency without degrading retrieval quality
type: project
---
...
- Post-retrieval pseudonymization (the architectural limit flagged in `rag.md:14`).
...
- Seb approval needed? **Yes** — for: (a) running `expand_query` concurrently with the pseudonymization/forwarding pipeline (changes request-path control flow); (b) any change to retriever `top_k` / reranker `top_k`; (c) introducing a cache (in-proc or Redis) for expansion results.

## Open questions
- [ ] Seb: target latency reduction number — is 40% end-to-end / 1500 ms expansion overhead the right bar?
- [ ] Seb: is it acceptable to run `expand_query` concurrently with user-message pseudonymization (both kick off as soon as we have the raw question), then await both before forwarding?
...
- [ ] Seb: OK to add a short-circuit that skips expansion when the question is already long/specific (heuristic: length > N chars OR contains a named entity)?
